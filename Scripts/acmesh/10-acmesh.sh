...
docker exec "$ACMESH_CONTAINER" --issue -d tnt.photo -d '*.tnt.photo' --server letsencrypt --dns dns_cf --force
docker exec "$ACMESH_CONTAINER" --issue -d linda-ebert.com -d '*.linda-ebert.com' --server letsencrypt --dns dns_cf --force
